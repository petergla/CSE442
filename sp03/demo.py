...
def fun1(label):
    ...
    for i in range(len(answer_key)):
        if answer_key[i] == answers[i]:
            score = score + 1
    label.configure(text =  "Current grade is " + str(score))
...
widthsum = scl1.width()+scl2.width()
quarterofwidthsum = int(widthsum/6)
can3 = Canvas(root)
can3.config(width=quarterofwidthsum,height=1)
can3.grid(row=2,column=1)




labelq = Label(root,text="Please select the correct assumptions:",fg='blue')
labelq.grid(row=2,column=1,sticky=W)
...
check1.grid(row=3,column=1,columnspan=2,sticky=W)
check2.grid(row=4,column=1,columnspan=2,sticky=W)
check3.grid(row=5,column=1,columnspan=2,sticky=W)
check4.grid(row=6,column=1,columnspan=2,sticky=W)

buttons = Button(root,text="Submit Answer",command=lambda: fun1(labelg))
buttons.grid(row=7,column=2,sticky=E)
