...
def makelayout(score, root, questionlist):
    ...
    for x in xrange(0, Nassumptions):
        ...
        for y in xrange(0, Nreasons):
            ...
            radiobutton = Radiobutton(frame2, text=question.getAssumptions()[x].getReasons()[y][1],
                                      variable=radiobuttonanswer[x], value=y + 1)
            # store in the grid
...
def firstsubmission(score, root, labelg, checkboxanswer, radiobuttonanswer, questionlist, gridlist, rowstorage, frame1,
                    frame2):
    ...
    for x in xrange(0, len(checkboxanswer)):
        ...
        if question.getAssumptions()[x].getTruthValue() == 0:
            ...
            if checkboxanswer[x].get() == 1:
                ...
                gridlist[len(gridlist) - 1].config(text='Submit Answer',
                                                   command=lambda: secondsubmission(score, root, labelg, checkboxanswer,
                                                                                    radiobuttonanswer, questionlist,
                                                                                    gridlist, rowstorage, frame1,
                                                                                    frame2))
                break


def secondsubmission(score, root, labelg, checkboxanswer, radiobuttonanswer, questionlist, gridlist, rowstorage, frame1,
                     frame2):
    # get the first question
    question = questionlist[0]
...
def finishpage(score, root):
    frame3 = Frame(width=500, height=500)
    frame3.pack(fill='both')
    photo = resizephotoimagewithin1000pixel(PhotoImage(file="codebusters.gif"))
